$(document).ready(function () {
  ...
  if (savedType) {
    document.querySelector(
      `input[name="attendanceType"][value="${savedType}"]`
    ).checked = true;
  }
  ...
  tbody.innerHTML =
    '<tr><td colspan="5" class="text-center text-muted">Select date and type, then click Apply to load attendance data</td></tr>';

  // If single type is selected (default or from localStorage), load employee dropdown
  const selectedType = document.querySelector(
    'input[name="attendanceType"]:checked'
  );
  ...
  $('input[name="attendanceType"]').change(function () {
    ...
    if (type === "single") {
      ...
      tbody.innerHTML =
        '<tr><td colspan="5" class="text-center text-muted">Click Apply to load attendance data</td></tr>';
    }
  });
    ...
    .then((response) => {
      if (response.success) {
        ...
        if (!response.employees || response.employees.length === 0) {
          tbody.innerHTML =
            '<tr><td colspan="5" class="text-center">No employees available for attendance</td></tr>';
          hideLoader();
          return;
        }
...
function loadAvailableEmployees() {
  ...
  return fetch(
    `includes/get_available_employees.php?date=${date}&_=${new Date().getTime()}`
  )
    .then((response) => response.json())
    .then((response) => {
...
function showSuccess(message) {
  const container =
    document.querySelector(".message-container") || createMessageContainer();
